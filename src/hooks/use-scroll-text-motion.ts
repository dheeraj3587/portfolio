...
function scramble(
  ...
  const duration =
    options?.duration ??
    parseFloat(el.dataset.scrambleDuration ?? "1") ??
    1;
...
export function useScrollTextMotion(
  ...
  useLayoutEffect(() => {
    ...
    // Force ScrollTrigger to re-measure after layout has settled.
    // Without this, triggers initialised against a stale layout (e.g. when
    // the component swaps in to replace another tree) can fail to fire.
    const refreshAfterPaint = () => {
      ScrollTrigger.refresh(true);
    };
    const r1 = requestAnimationFrame(() =>
      requestAnimationFrame(refreshAfterPaint),
    );
    const r2 = window.setTimeout(refreshAfterPaint, 250);
  ...
  }, [contentRef, logoRef, groups, logo]);
}
